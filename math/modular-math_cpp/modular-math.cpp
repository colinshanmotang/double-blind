...
void modularReduction_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void modularReduction_3_run(uint ctx_index,Circom_CalcWit* ctx);
void modularExponentiation_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void modularExponentiation_4_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[5] = { 
Num2Bits_0_run,
LessThan_1_run,
GreaterEqThan_2_run,
modularReduction_3_run,
modularExponentiation_4_run };
...
uint get_total_signal_no() {return 1194;}

uint get_number_of_components() {return 49;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 1099;}
...
ctx->componentMemory[coffset].templateName = "modularReduction";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
{
std::string new_cmp_name = "geqzero";
GreaterEqThan_2_create(mySignalStart+4,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
std::string new_cmp_name = "ltm";
LessThan_1_create(mySignalStart+76,3+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[1] = 3+ctx_index+1;
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_mod(&expaux[0],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 2]); // line circom 9
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_idiv(&expaux[0],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 2]); // line circom 10
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
Fr_mul(&expaux[1],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 2]); // line circom 11
Fr_sub(&expaux[2],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 0]); // line circom 11
{{
Fr_eq(&expaux[0],&expaux[1],&expaux[2]); // line circom 11
}}
...
cmp_index_ref_load = 1;
{{
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + 0],&circuitConstants[2]); // line circom 21
}}
...
ctx->componentMemory[coffset].templateName = "modularExponentiation";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 4; i++) {
std::string new_cmp_name = "modRed"+ctx->generate_position_array(aux_dimensions, 1, i);
modularReduction_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
...
uint csoffset = mySignalStart+613;
uint aux_dimensions[1] = {4};
for (uint i = 0; i < 4; i++) {
std::string new_cmp_name = "modRed2"+ctx->generate_position_array(aux_dimensions, 1, i);
modularReduction_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+ i] = aux_cmp_num;
csoffset += 145 ;
aux_cmp_num += 6;
...
while(Fr_isTrue(&expaux[0])){
{
Fr_sub(&expaux[2],&circuitConstants[2],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 2)]); // line circom 66
Fr_mul(&expaux[1],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 2)],&expaux[2]); // line circom 66
{{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 24)],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 24)]); // line circom 71
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 11)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
modularReduction_3_run(mySubcomponents[cmp_index_ref],ctx);
...
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 15)];
// load src
...
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 7)];
// load src
Fr_sub(&expaux[3],&signalValues[mySignalStart + 1],&circuitConstants[2]); // line circom 76
Fr_sub(&expaux[4],&circuitConstants[6],&lvar[2]); // line circom 76
Fr_mul(&expaux[2],&expaux[3],&signalValues[mySignalStart + ((1 * Fr_toInt(&expaux[4])) + 2)]); // line circom 76
Fr_add(&expaux[0],&circuitConstants[2],&expaux[2]); // line circom 76
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 19)];
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 15)],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 7)]); // line circom 77
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * (Fr_toInt(&lvar[2]) + 1)) + 24)];
// load src
...
modularExponentiation_4_create(1,0,ctx,"main",0);
modularExponentiation_4_run(0,ctx);
}
